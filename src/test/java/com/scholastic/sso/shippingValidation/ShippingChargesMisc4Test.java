...
public class ShippingChargesMisc4Test extends TestBase{
	...
	public void shippingChargesMisc4Test(Hashtable<String, String> table) throws IOException, InterruptedException{
		...
		APPLICATION_LOG.debug("******** Executing -  Shipping Charges Misc4 Test ********");
		validateRunmodes("ShippingChargesMisc4Test", Constants.SHIPPINGVALIDATION_SUITE, table.get(Constants.RUNMODE_COL));
		...
		if (shippingCharges == calcShippingCharges) {
			...
			APPLICATION_LOG.debug("ExeComments --- Shipping Charges NOT matching :( for Standard_Shipping with 4 Books");
		...
		if (shippingCharges1 == calcShippingCharges1) {
			APPLICATION_LOG.debug("ExeComments --- Both Calculated and Captured Shipping Charges MATCHING for Two_Day_Shipping with 4 Books");
		} else {
			APPLICATION_LOG.debug("ExeComments --- Shipping Charges NOT matching :( for Two_Day_Shipping with 4 Books");
			fail("Shipping Charges NOT matching :( for Standard_Shipping");
		}
		...
		if (shippingCharges2 == calcShippingCharges2) {
			APPLICATION_LOG.debug("ExeComments --- Both Calculated and Captured Shipping Charges MATCHING for One_Day_Shipping with 4 Books");
		} else {
			APPLICATION_LOG.debug("ExeComments --- Shipping Charges NOT matching :( for One_Day_Shipping for 4 Books");
			fail("Shipping Charges NOT matching :( for Standard_Shipping");
		}
